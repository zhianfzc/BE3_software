...
static int st_mod_to_host_reply(uint8_t nid, uint8_t reply_result)
{
	...
	s_msgtag *pst_msg = (s_msg*)malloc(msg_buf_len);
	
	memset(pst_msg, 0, msg_buf_len);
	memset(&pst_msg->tag, 0x55, 5);//zcy add
	memset(&pst_msg->tag+5, 0x00, 5);//zcy add
	msg_len += 10;
	...
	if (g_enc_mode == 0x2)
	{
		...
		msg_len+=2;
		msg_len+=10;
		s_enc_msgtag *pst_enc_msg = (s_enc_msgtag*)malloc(msg_len);
		memset(pst_enc_msg, 0, msg_len);
		memset(&pst_enc_msg->tag, 0x55, 5);//zcy add
		memset(&pst_enc_msg->tag+5, 0x00, 5);//zcy add
		...
		msg_len+=10;
		s_enc_msgtag *pst_enc_msg = (s_enc_msgtag*)rt_malloc(msg_len);
		memset(pst_enc_msg, 0, msg_len);
	...
	else
	{
		...
		for (i=2+10; i<msg_len-1; i++)//zcy add tag len 10
		{
