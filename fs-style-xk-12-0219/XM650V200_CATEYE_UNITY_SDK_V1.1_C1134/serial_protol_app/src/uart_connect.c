...
static int st_mod_recv_host_msg()
{
	...
	else
	{
		msg_len = sizeof(s_msg);
			//get DE AA
		uint8_t tmp[3] ={0};
		ret = host_msg_get((char*)tmp, timeout, 3);
		
		if (tmp[0] == 0xDE && tmp[1] == 0xAA) 
		{
			st_recv_msg.mid = tmp[2];
			
			ret = host_msg_get((char*)&st_recv_msg.size_heb, timeout, 2);
			if (ret != 0) 
			{
				MY_ERR_PRT("host_msg_get", ret);
				return MY_ERRROR;
			...
			st_recv_msg.mid = tmp[0];
			st_recv_msg.size_heb = tmp[1];
			st_recv_msg.size_leb = tmp[2];
		}
...
{
	unsigned char *p_face_power_sign = face_power_sign();
	//usleep(150*1000);	//fcz add for wait 150MS
