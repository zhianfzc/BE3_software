...
typedef void (*ARM_USBH_ISOCH_DATA_CALLBACK)(uint32_t *payload, uint32_t length);
typedef uint32_t (*ARM_USBH_ISOCH_BH_CALLBACK)();                                           ///< Bottom-half callback,  return value is not important
typedef uint32_t (*ARM_USBH_ISOCH_ITD_WORK_FUNC)();                                             ///< iTD work process function for user bottom-half callback
#endif
...
#ifdef KNERON_USBH_MDW
  ARM_USBH_PIPE_HANDLE  (*PipeCreate_ISOCH)  (uint8_t dev_addr,
                                                  uint8_t ep_addr,
                                                  ...
                                                  uint8_t *buf,
                                                  uint32_t buf_size);
  int32_t               (*PipeStart_ISOCH)       (ARM_USBH_PIPE_HANDLE pipe_hndl, ARM_USBH_ISOCH_DATA_CALLBACK isoch_data_cb);
  int32_t               (*PipeStop_ISOCH)        (ARM_USBH_PIPE_HANDLE pipe_hndl);
  ARM_USBH_ISOCH_ITD_WORK_FUNC  (*PipeEnableBH_ISOCH) (ARM_USBH_PIPE_HANDLE pipe_hndl, ARM_USBH_ISOCH_BH_CALLBACK isoch_bh_callback);
#endif
...
extern ARM_DRIVER_USBH Driver_USBH0; // USBH driver instance
