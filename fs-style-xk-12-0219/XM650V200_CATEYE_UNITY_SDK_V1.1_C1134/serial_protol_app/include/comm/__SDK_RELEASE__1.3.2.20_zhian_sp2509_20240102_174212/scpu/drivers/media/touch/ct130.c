...
int _ct130_init(struct core_device *core_d)
{
    int ret;
...
u16 _ct130_read_touch_id(struct core_device *core_d)
{
    u16 id = 0;
    //TODO : read ID
    return id;
...
void _ct130_start(struct core_device *core_d)
{
    struct touch_driver *touch_drv = &kdp520_touch_driver;
    struct touch_driver_ops *ops = (struct touch_driver_ops*)touch_drv->ctrller_ops;
    ops->tp_gpio_set_int_start();
}
void _ct130_stop(struct core_device *core_d)
{
    struct touch_driver *touch_drv = &kdp520_touch_driver;
    struct touch_driver_ops *ops = (struct touch_driver_ops*)touch_drv->ctrller_ops;
...
void _ct130_get_raw_data(struct core_device *core_d, u8 init_i2c)
{
    u8 buf[30] = {0};
    u8 i2c_buf[8] = {0};
...
static u16 _ct130_get_info(struct core_device * core)
{
    u8 dev_addr = 0x1A;
    u16 regAddr = 0;
    u8 buf1[20] = {0};
    ...
    regAddr = 0xD101;
    kdp_drv_i2c_write(I2C_ADAP_0, dev_addr, regAddr, 2, buf1[2]);
...
void _ct130_state_handler(struct core_device *core_d, void* pData)
{
...
void _ct130_set_inactive(void) {
    __touch_data.count = 0;
}
int _ct130_get_active(void) {
...
struct touch_panel_driver ct130_driver = {
    .init             = _ct130_init,
    .start            = _ct130_start,
    .stop             = _ct130_stop,
    .get_raw_data     = _ct130_get_raw_data,
    .state_handler    = _ct130_state_handler,
    .set_inactive     = _ct130_set_inactive,
    .get_active       = _ct130_get_active,
    .read_did         = _ct130_get_info,
};
#endif
