...
#include "flash.h"
#if (FLASH_VENDOR_SELECT == W25Q256JV )
#include "W25Q256JV.h"
#include "types.h"
#include "base.h"
#include "delay.h"
...
void norflash_write_control_volatile(UINT8 enable)
{
    /* fill in command 0~3 */
    if (enable) {
        kdp520_spi_set_commands(WRITE_CON_50_CMD0, WRITE_CON_50_CMD1, WRITE_CON_50_CMD2, WRITE_CON_50_CMD3);
    } else {
        kdp520_spi_set_commands(WRITE_CON_04_CMD0, WRITE_CON_04_CMD1, WRITE_CON_04_CMD2, WRITE_CON_04_CMD3);
    }
    /* wait for command complete */
...
void nor_flash_quad_mode_en(UINT8 enable)
{
    ...
    norflash_write_control_volatile(1);
...
kdp_status_t norflash_get_info(void)
{
    ...
    if((st_flash_info.erase_4K_support == 1) &&
        (st_flash_info.flash_size_KByte == 16383) &&
        (st_flash_info.sector_size_Bytes == 4096) &&
        (st_flash_info.total_sector_numbers == 4096) &&
        (st_flash_info.block_size_Bytes == 16) &&
        (st_flash_info.page_size_Bytes == 256 ) )
    {
        flash_4byte_en = false;
        dbg_msg_console("FLASH_SIZE: 16M");
    }
    else
#endif
#if (FLASH_SIZE >= FLASH_32MB)
    if((st_flash_info.erase_4K_support == 1) &&
        (st_flash_info.flash_size_KByte == 32767) &&
        (st_flash_info.sector_size_Bytes == 4096) &&
        (st_flash_info.total_sector_numbers == 8192) &&
        (st_flash_info.block_size_Bytes == 16) &&
        (st_flash_info.page_size_Bytes == 256 ) )
    {
        flash_4byte_en = true;
        dbg_msg_console("FLASH_SIZE: 32M");
    }
    else
#endif
    {
        dbg_msg_console("[FIX]flash or driver must be repaired");
        dbg_msg_console("erase_4K_support:%d", st_flash_info.erase_4K_support);
        dbg_msg_console("flash_size_KByte:%d", st_flash_info.flash_size_KByte);
        dbg_msg_console("sector_size_Bytes:%d", st_flash_info.sector_size_Bytes);
        dbg_msg_console("total_sector_numbers:%d", st_flash_info.total_sector_numbers);
        dbg_msg_console("block_size_Bytes:%d", st_flash_info.block_size_Bytes);
        dbg_msg_console("page_size_Bytes:%d", st_flash_info.page_size_Bytes);
        while (1);
    }
...
kdp_status_t address_protect_check( UINT32 addr_start, UINT32 addr_end, UINT32 pro_start, UINT32 pro_end)
{
    if ( protect_bypass == 0 )
    {
        ...
        {
            dbg_msg_console ("trying to modify flash:%x,%x.\n", addr_start, addr_end);
            return KDP_STATUS_ERROR;
        }
    }
    return KDP_STATUS_OK;
...
kdp_status_t norflash_4k_erase(UINT32 address)
{
    ...
    if ( model_protect_check( address, (address + 4096) ) == KDP_STATUS_ERROR )
    {
        return KDP_STATUS_ERROR;
    }
    ...
    if (flash_4byte_en)
        kdp520_spi_set_commands( address , ERASE_4K_21_CMD1, ERASE_4K_21_CMD2, ERASE_4K_21_CMD3);
    else
        kdp520_spi_set_commands( address , SPI020_20_CMD1, SPI020_20_CMD2, SPI020_20_CMD3);
    kdp520_spi_wait_command_complete();
...
kdp_status_t norflash_32k_erase(UINT32 offset)
{
    ...
    /*Protect FW_INFO and ALL_MODELS*/
    if ( model_protect_check( offset, (offset + 32768) ) == KDP_STATUS_ERROR )
    {
        return KDP_STATUS_ERROR;
    }
    
    norflash_write_control(1);/* send write enabled */
    kdp520_spi_set_commands(offset, ERASE_32K_52_CMD1, ERASE_32K_52_CMD2, ERASE_32K_52_CMD3);
    kdp520_spi_wait_command_complete();
...
kdp_status_t norflash_64k_erase(UINT32 offset)
{
    ...
    /*Protect FW_INFO and ALL_MODELS*/
    if ( model_protect_check( offset, (offset + 65536) ) == KDP_STATUS_ERROR )
    {
        return KDP_STATUS_ERROR;
    }
    ...
    if (flash_4byte_en)
        kdp520_spi_set_commands(offset, ERASE_64K_DC_CMD1, ERASE_64K_DC_CMD2, ERASE_64K_DC_CMD3);
    else
        kdp520_spi_set_commands(offset, SPI020_D8_CMD1, SPI020_D8_CMD2, SPI020_D8_CMD3);
    kdp520_spi_wait_command_complete();
...
kdp_status_t norflash_read(uint32_t addr, void *data, uint32_t target_Bytes)
{
    ...
    write_addr = (u32)data;
    //rx_fifo_depth = (s32)kdp520_spi_rxfifo_depth();
    ...
    while (total_lens > 0)
    {
        ...
        if (flash_4byte_en)
        {
            ...
            #ifdef SPI_QUAD_MODE
                kdp520_spi_set_commands( (uint32_t)(addr) , QUAD_READ_EC_CMD1, DMA_LIMIT, QUAD_READ_EC_CMD3 );
            #else
        ...
        {
            #ifdef SPI_QUAD_MODE
                kdp520_spi_set_commands( (uint32_t)(addr) , SPI020_EB_CMD1, DMA_LIMIT, SPI020_EB_CMD3 );
            #else
                kdp520_spi_set_commands( (uint32_t)(addr) , SPI020_03_CMD1, DMA_LIMIT, SPI020_03_CMD3 );
            #endif
        }
        ...
        if (flash_4byte_en)
        {
            norflash_4Bytes_ctrl(1);
            #ifdef SPI_QUAD_MODE
                kdp520_spi_set_commands( (uint32_t)(addr) , QUAD_READ_EC_CMD1, total_lens, QUAD_READ_EC_CMD3 );
            #else
                #if SPI_BUS_SPEED == SPI_BUS_SPEED_100MHZ
                    dbg_msg_err("[%s] err, 13H cmd can not run on 100MHz");
                #else
        ...
        {
            #ifdef SPI_QUAD_MODE
                kdp520_spi_set_commands( (uint32_t)(addr) , SPI020_EB_CMD1, total_lens, SPI020_EB_CMD3 );
            #else
                kdp520_spi_set_commands( (uint32_t)(addr) , SPI020_03_CMD1, total_lens, SPI020_03_CMD3 );
            #endif
        }
...
void norflash_write_running( UINT8 type, UINT32 offset, UINT32 total_send_byte, UINT8 *buf )
{
    ...
    if(type & FLASH_QUAD_RW)
    {
        ...
        if (flash_4byte_en)
            kdp520_spi_set_commands(offset, QPAGE_WRITE_34_CMD1, total_send_byte, QPAGE_WRITE_34_CMD3);
        else
        #endif
        {
    ...
    {
        if (flash_4byte_en)
            kdp520_spi_set_commands(offset, PAGE_WRITE_12_CMD1, total_send_byte, PAGE_WRITE_12_CMD3);
        else
...
#endif  //end if #if (FLASH_VENDOR_SELECT == W25Q256JV )
