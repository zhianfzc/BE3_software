...
#include "flash.h"
#if (FLASH_VENDOR_SELECT == W25M512JV )
#include "W25M512JV.h"
#include "types.h"
#include "base.h"
#include "delay.h"
...
kdp_status_t norflash_program( UINT32 addr, UINT8 *data, UINT32 send_bytes)
{
    ...
    for( i=0; i<pages_number ; i++  )
    {
        ...
        //dbg_msg_flash("flash program sector: %d", i);
    }
    if( nbytes_num > 0 )
    {
    ...
    //dbg_msg_flash("flash program final size: %d", nbytes_num);
...
#endif  //end if #if (FLASH_VENDOR_SELECT == W25M512JV )
