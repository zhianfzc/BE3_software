...
int keyExpansion(const uint8_t *key, uint32_t keyLen, AesKey *aesKey) {
    ...
    if (NULL == key || NULL == aesKey){
        dbg_msg_console("keyExpansion param is NULL\n");
        return -1;
    }

    if (keyLen != 16){
        dbg_msg_console("keyExpansion keyLen = %d, Not support.\n", keyLen);
        return -1;
    }
...
int aesEncrypt(const uint8_t *key, uint32_t keyLen, const uint8_t *pt, uint8_t *ct, uint32_t len) {
    ...
    uint8_t state[4][4] = {0};
		//dbg_msg_console("aesEncrypt");
    if (NULL == key || NULL == pt ){//|| NULL == ct){
        dbg_msg_console("param err.");
        return -1;
    }

    if (keyLen > 16){
        dbg_msg_console("keyLen must be 16.");
        return -1;
    }

    if (len % BLOCKSIZE){
        dbg_msg_console("inLen is invalid.");
        return -1;
    }

    memcpy(actualKey, key, keyLen);
		//dbg_msg_console("1111");
    keyExpansion(actualKey, 16, &aesKey);
//dbg_msg_console("2222");
    for (int i = 0; i < len; i += BLOCKSIZE) {

        loadStateArray(state, pt);
        addRoundKey(state, rk);
//dbg_msg_console("33333");
        for (int j = 1; j < 10; ++j) {
            rk += 4;
            subBytes(state);
            shiftRows(state);
...
int aesDecrypt(const uint8_t *key, uint32_t keyLen, const uint8_t *ct, uint8_t *pt, uint32_t len) {
    ...
    uint8_t actualKey[16] = {0};
    uint8_t state[4][4] = {0};
		//dbg_msg_console("aesDecrypt coming in.");
    ...
    //dbg_msg_console("aesDecrypt coming out.");
    return 0;
}
#endif
