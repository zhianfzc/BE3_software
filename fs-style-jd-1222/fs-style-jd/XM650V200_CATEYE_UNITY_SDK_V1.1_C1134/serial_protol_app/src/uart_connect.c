...
static int st_mod_to_host_reply(uint8_t nid, uint8_t reply_result)
{
	...
	s_msg_reply_data *pst_msg_reply = (s_msg_reply_data*)pst_msg->data;
...
int st_mod_to_host_mid(uint8_t mid, uint8_t nid, uint8_t reply_result)
{
	...
	switch (mid)
	{
		case MID_REPLY:
			...
			ret = st_mod_to_host_reply(nid, reply_result);
			if (ret != 0)
			{
...
static int st_host_to_mod_mid(s_msg *pst_mod_recv_msg)
{
	...
	switch(pst_mod_recv_msg->mid)
	{
		...
		case MID_ENROLL_ITG:
		{
			...
			//zcy add 
			if(pst_enroll_itg->enable_duplicate ==0)
				pst_enroll_itg->enable_duplicate = 1;
			else
				pst_enroll_itg->enable_duplicate = 0;
		...
		case MID_INIT_ENCRYPTION:
		{
			...
			else/* AES128 加密模式 暂不支持*/
			{
				if (st_mod_to_host_mid(MID_REPLY, MID_INIT_ENCRYPTION, MR_FAILED4_UNKNOWNREASON) != 0)
					MY_ERR_PRT("st_mod_to_host_mid", MY_ERRROR);
			}
                  #endif
			break;
...
#if 0 //zcy del 20230927
			while (1)
			{
				MY_STATUS_PRT("please power down");
				sleep(60);
			}
#endif
...
static int st_mod_recv_host_msg()
{
	...
	while (1)
	{
		...
		{
			MY_ERR_PRT("sync_word error", st_recv_msg.sync_word_leb);
			continue;
		}
